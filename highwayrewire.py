	...
	for ri,fi,iid in getIntersections():
		...
		def shootToWay(tw):
			return functools.reduce(min,
				((l,ti) for ti,(l,s) in (
					(i,osmcmd.Segment(rpt1,rpt2).intersect(osmcmd.Segment.fromNodes(
						data.nodes[tw[OsmData.REF][i]],
						data.nodes[tw[OsmData.REF][i+1]]
		...
		# move node
		(rpt1+(rpt2-rpt1)*lr).setNode(data.nodes[iid])
		# add node 'to'
		...
		nid,_=(rpt1+(rpt2-rpt1)*ls).makeNode(data)
